...
dcph = read_csv('data_input/Lakes and Outfalls - Chloride Data.csv') |> 
  select(Lake, Date = Date_Sampled, cl = Results)
table(dcph$Lake)

ntl = read_csv('data_input/ntl2_v10.csv') |> 
  filter(depth == 0) |> 
...
read_csv('data_input/ntl2_v10.csv') %>% filter(lakeid == 'WI') %>% 
  filter(cl == max(cl, na.rm = T))

read_csv('data_input/ntl2_v10.csv') %>% filter(lakeid == 'WI') %>% 
  filter(depth > 2) %>% select(sampledate, depth, cl)

older = read_csv('data_input/MadisonLakes_chloride[38].csv') |> 
