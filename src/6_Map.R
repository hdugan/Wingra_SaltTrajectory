...
ggplot() +
  ...
  geom_sf(data = metstation, color = "white", size = 1) +
...
ggplot() +
  ...
  geom_raster(data = landuse_merge, 
              aes(x = x, y = y, fill = factor(landuse))) +
  geom_sf(data = is_wingra.roads, color = 'black', fill = 'black') +
  geom_sf(data = is_wingra.paved, linewidth = 0.1, color = 'black', fill = 'black') +
  geom_sf(data = wingraWS, fill = NA, color = "black", linewidth = 0.5) +
  geom_sf(data = lakewingra, fill = '#5475A8') +
  # geom_sf(data = metstation, color = "white", size = 1) +
  annotate('text',x = 532800, y = 2247150, label = "Lake Wingra", 
...
ggsave('Figures_new/Map_2023roads.png', width = 6, height = 2.5, dpi = 500)
